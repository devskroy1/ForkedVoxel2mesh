...
from data.brainMRI import BrainMRI
...
def load_config(exp_id):
    ...
    **************************************** Paths ****************************************
    ...
    '''
    cfg.save_path = "/vol/bitbucket/sr4617/ForkedVoxel2mesh/results/"  # UPDATE HERE <<<<<<<<<<<<<<<<<<<<<<
    cfg.dataset_path = "/vol/biomedic/users/aa16914/shared/data/dhcp_neonatal_brain/gm_volume3d/"  # UPDATE HERE <<<<<<<<<<<<<<<<<<<<<<
    ...
    cfg.data_obj = BrainMRI()  # UPDATE HERE <<<<<<<<<<<<<<<<<<<<<<
    # cfg.data_obj = Chaos()
